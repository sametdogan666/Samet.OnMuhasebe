...
public class EfCoreCommonRepository<TEntity> : EfCoreRepository<OnMuhasebeDbContext, TEntity, Guid>, ICommonRepository<TEntity> where TEntity : class, IEntity<Guid>
{
    ...
    public async Task<TEntity> GetAsync(object id, Expression<Func<TEntity, bool>>? predicate = null, params Expression<Func<TEntity, object>>[] includeProperties)
    {
        ...
        if (predicate != null)
        {
            entity = await queryable.FirstOrDefaultAsync(predicate) ?? throw new EntityNotFoundException(typeof(TEntity), id);

            return entity;
        }

        entity = await queryable.FirstOrDefaultAsync() ?? throw new EntityNotFoundException(typeof(TEntity), id);
    ...
    public async Task<TEntity> GetAsync(Expression<Func<TEntity, bool>>? predicate = null, params Expression<Func<TEntity, object>>[] includeProperties)
    {
        ...
        return predicate != null
            ? await queryable.FirstOrDefaultAsync(predicate) ?? throw new EntityNotFoundException()
            : await queryable.FirstOrDefaultAsync() ?? throw new EntityNotFoundException();
    }
    ...
    public async Task<List<TEntity>> GetPagedListAsync<TKey>(int skipCount, int maxResultCount, Expression<Func<TEntity, bool>>? predicate = null,
    ...
    public async Task<List<TEntity>> GetPagedLastListAsync<TKey>(int skipCount, int maxResultCount, Expression<Func<TEntity, bool>>? predicate = null,
        Expression<Func<TEntity, TKey>>? orderBy = null, params Expression<Func<TEntity, object>>[] includeProperties)
    {
